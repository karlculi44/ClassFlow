...
describe("authentication schemas", () => {
  ...
  it("validates login and password-reset email input", () => {
    expect(
      loginSchema.safeParse({ email: "bad", password: "secret123" }).success,
    ).toBe(false);
    expect(
...
describe("class and assignment schemas", () => {
  ...
  it("rejects an invalid class time range and duplicate fields", () => {
    ...
    expect(
      createClassSchema.safeParse({ ...classData, extra: true }).success,
    ).toBe(false);
  });
  ...
  it("accepts valid assignment data and positive numeric params", () => {
    ...
    expect(createAssignmentParamsSchema.parse({ classId: "12" })).toEqual({
      classId: 12,
    });
  });
  ...
  it("rejects impossible assignment dates and invalid params", () => {
    ...
    expect(
      createAssignmentParamsSchema.safeParse({ classId: "0" }).success,
    ).toBe(false);
  });
});
...
describe("submission schemas", () => {
  it("normalizes grade and optional feedback", () => {
    expect(
      gradeSubmissionSchema.parse({ grade: " 95 ", feedback: "  Great work " }),
    ).toEqual({
      grade: 95,
      feedback: "Great work",
    });
  ...
  it("rejects grades outside 0-100 and invalid assignment IDs", () => {
    ...
    expect(
      submitAssignmentParamsSchema.safeParse({ assignmentId: "-1" }).success,
    ).toBe(false);
  });
});
